...
const Settings = () => {
  const dispatch = useDispatch()
    const {setShowSettings,showSettings, setShowEditBoardModal, selectedBoard} = useContext(KanbanContext)
  return (
    <>
      ...
      text-center p-3 rounded-md text-[12px] w-[150px] dark:text-gray-500 dark:bg-black transition-all duration-500 border top-12
       right-4 shadow-sm absolute`}>
          ...
          <p 
          // onClick={() =>  dispatch(deleteBoard(selectedBoard.id)) & setShowSettings(false)}
          className='text-red cursor-pointer w-full hover:font-bold'>Delete board</p>
