...
import { markCompleted } from "@/utils/redux/slice ";
 ...
 bg-black bg-opacity-10 scrollbar-hide"
    >
      <div className="bg-white min-h-[250px] dark:bg-gray-dark lg:w-[400px] md:w-[350px] h-fit w-[80%] p-4 rounded-md   absolute top-8">
        ...
        <Image
          className="cursor-pointer w-[5px] absolute right-4 top-10"
          src={settings}
          alt="settings"
        ...
        <div className="text-gray-500">
          ...
          {task?.subtasks.map((subtask, index) => (
            <div
              ...
              className= {` ${subtask.isCompleted ? "bg-blue-lightest" : "bg-blue-lighter" } ease-in-out cursor-pointer duration-300 flex hover:text-black hover:bg-purple-light items-center gap-4 p-2 mt-3 rounded-md`}
            >
              <div
             
                className="border w-[15px] relative h-[15px] flex  rounded-sm cursor-pointer"
              >
                {subtask.isCompleted && (
              ...
              <span className={`${subtask.isCompleted ? "line-through" : ""} text-[10px]`}>{subtask.title}</span>
            </div>
          ))}
        </div>
